...
// Ask the user for the molecule name
Dialog.create("Molecule Information");
Dialog.addString("Enter the molecule name (e.g., TDP):", "TDP");
Dialog.show();
molecule = Dialog.getString();
...
Dialog.addNumber("Which channel number is " + molecule + "?", 1);
Dialog.addNumber("Which channel number is " + maskName + "?", 2);

Dialog.addChoice("Thresholding method - " + molecule, 
...
Dialog.addNumber("Threshold minimum (" + molecule + " channel)", 0);
Dialog.addNumber("Threshold maximum (" + molecule + " channel)", 65535);

Dialog.addChoice("Thresholding method - " + maskName, 
    newArray("Default", "Huang", "Intermodes", "IsoData", "Li", "MaxEntropy", "Mean", 
             "MinError(I)", "Minimum", "Moments", "Otsu", "Percentile", "RenyiEntropy", 
             "Shanbhag", "Triangle", "Yen"), "Default");
...
Dialog.addCheckbox("Quality control?", false);
Dialog.addNumber("Mean cut-off (0 to 1)", 0.95);

Dialog.addHelp("https://github.com/PerlsonLab/Bioprotocol");

Dialog.show();

moleculeIndex = Dialog.getNumber();
maskIndex = Dialog.getNumber();
thresholdMethodMolecule = Dialog.getChoice();
thresholdMinMolecule = Dialog.getNumber();
thresholdMaxMolecule = Dialog.getNumber();
thresholdMethodMask = Dialog.getChoice();
thresholdMinMask = Dialog.getNumber();
thresholdMaxMask = Dialog.getNumber();
qualityControlBoolean = Dialog.getCheckbox();
meanCut = Dialog.getNumber();
updatedMeanCut = meanCut*255;

// Define base output folder name
baseOutputDir = inputDir + "masked_" + molecule + "Min" + thresholdMinMolecule + "Max" + thresholdMaxMolecule + "_" + maskName + "Min" + thresholdMinMask + "Max" + thresholdMaxMask;
...
for (i = 0; i < fileList.length; i++) {
    if (endsWith(fileList[i], ".ims") || 
        ...
        // Work on copy for mask generation
        selectImage(imageTitles[maskIndex - 1]);
        ...
        // Select molecule channel
        moleculeTitle = imageTitles[moleculeIndex - 1];
        selectImage(moleculeTitle);
        ...
        deletedCountMolecule = 0;
        deletedCountMask = 0;
        deletedIndices = "";
		if (qualityControlBoolean == true) {
	        ...
	        for (s = stackSize; s >= 1; s--) {
	            ...
	            if (mean >= updatedMeanCut) {
	            	// Delete from molecule channel
	                selectImage(moleculeTitle);
	                setSlice(s);
	                run("Delete Slice");
	                deletedCountMolecule++;
        ...
        // Create mask on molecule channel
        selectImage(moleculeTitle);
        setThreshold(thresholdMinMolecule, thresholdMaxMolecule);
        setOption("BlackBackground", true);
        run("Convert to Mask", "method=" + thresholdMethodMolecule + " background=Dark black create");
        moleculeMaskedTitle = getTitle();

        // Subtract mask from molecule channel
        imageCalculator("Multiply create stack", moleculeMaskedTitle, maskTitle);
        resultTitle = getTitle();

        // Z project: Molecule
        selectImage(resultTitle);
        run("Z Project...", "projection=[Max Intensity]");
        saveAs("Tiff", moleculeFolder + "/" + fileName + "_" + molecule + ".tif");
		...
		if (deletedCountMask > 0) {
            deletionLog += fileName + ": Deleted " + deletedCountMask + " slice(s).";
        }
...
savedFiles = getFileList(moleculeFolder);

if (savedFiles.length == 0) {
    // Case: Output folder is empty (No images processed)
    showMessage("Warning", "No images were processed!\n\nPossible reasons:\n- Input folder is empty\n- Files do not match supported extensions (.ims, .tif, .czi, etc.)");
} else {
    // Case: Images were successfully saved
